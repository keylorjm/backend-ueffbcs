...
exports.iniciarSesion = async (req, res, next) => {
    ...
    if (!correo || !clave) {
        return next(new ErrorResponse('Por favor, proporciona un correo y una contraseña', 400));
    }
...
exports.recuperarContrasena = async (req, res, next) => {
    ...
    try {
        ...
        if (!usuario) {
            ...
            return res.status(200).json({ success: true, data: 'Instrucciones enviadas al correo si existe.' });
        }

        const resetToken = usuario.getResetPasswordToken(); 
        await usuario.save({ validateBeforeSave: false });

        // URL del FRONTEND para restablecer la clave (DEBES AJUSTAR ESTO)
        // La URL de tu frontend es: http://localhost:4200/restablecer-contrasena/:token
        ...
        res.status(200).json({ success: true, data: 'Instrucciones enviadas al correo.' });
        ...
        return next(new ErrorResponse('Error al intentar enviar el correo. Verifica las credenciales en .env', 500));
    }
};

// @desc    Restablecer Contraseña (Finaliza el proceso)
// @route   PUT /api/autenticacion/restablecerContrasena/:resetToken
exports.restablecerContrasena = async (req, res, next) => {
    // Hashear el token recibido por la URL
    ...
    try {
        ...
        usuario.clave = req.body.clave; // El pre-save hook en Usuario.js se encarga de hashearla
        usuario.resetPasswordToken = undefined;
        usuario.resetPasswordExpire = undefined;

        await usuario.save(); 

        sendTokenResponse(usuario, 200, res); // Inicia sesión automáticamente con el nuevo token
